...
PORT = 8888
server_ip = '10.222.179.79'
# server_address = ('47.251.161.108', PORT)
...
def download_file():
    ...
    """
    global client_socket, now_seqnum_send, now_seqnum_recv, expectedseqnum, drop_flag, temp, server_address
    ...
    while True:
        try:
            # Wait for ACK for the filename request
            ack, addr = client_socket.recvfrom(1024)
            ack_obj = Packet_to_Object(ack)
            print(f"Received from {addr}: ACK = {ack_obj.ack}")

            if ack_obj.ack == 1:
                # Successfully received ACK, continue with the rest of the logic
                base_end = time.time()
                print('Received ACK for filename')
                break  # Exit the loop once we receive a valid ACK for the filename
            else:
                print("No such file or incorrect ACK.")
                break  # If server responds with something other than ack=1, stop further retries
        except socket.timeout:
            # If timeout occurs, resend the filename request packet
...
if __name__ == '__main__':
    while True:
        client_socket.sendto(GBN_SR(seqnum=now_seqnum_send, fin=3).to_packet(), server_address)
        client_socket.settimeout(2)  # Set timeout to 2 seconds, adjust as needed
        try:
            new_port_data,  addr= client_socket.recvfrom(1024)
            new_port = int(new_port_data.decode())  # 获取新的端口号
            server_address = (server_ip, new_port)
            break
        except socket.timeout:
            print('Timeout waiting for newport of filename, resending...')
